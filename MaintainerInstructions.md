...
- [`QUESTS.md`](QUESTS.md) — locked 15-line guidance roadmap. **Create Core** has
  shipped. Committed `.snbt` under `pack/config/ftbquests/` is the source of truth
  (`bigGlobeAero/build_ftbquests.py`). IDs are SHA-1 of stable keys (0.8.1).
...
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.8.6.
...
- Giant towers: `keep_kayra` / `infested_temple` / `kisegi_sanctuary` start **0** +
  ...
  current. Compat history is in CHANGELOG [0.7.0]–[0.8.6].
...
**0.8.6**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
`bigGlobeAero/build_ftbquests.py`. IDs are SHA-1 of stable keys (0.8.1). Do not
overwrite via in-game edit mode.
...
roadmap. **FTB Quests is in the pack** (Create Core shipped). When adding a mod,
...
### packwiz `side` (as of 0.5.2–0.8.6)
