...
### packwiz `side` (as of 0.5.2–0.5.4)
...
**JEI and MapStitch must stay `side = "both"`.** 0.5.2 briefly marked both client-only; **0.5.3**
reverted JEI and **0.5.4** reverted MapStitch so dedicated servers still get them. Do not lump
either in with the rendering/QoL client-only list.
