...
Current pack version in `pack.toml` is **0.6.6**. **Fresh world required** for players upgrading
from any version before **0.6**. 0.6.1–0.6.6 are datapack-only (ocean glaciers) and do not
need a new world if they are already on 0.6.

## Height-patched Big Globe (as of 0.6, still in 0.6.6)
...
**Ocean glaciers** (`pack/datapacks/bigglobe_less_glacier.zip`, as of **0.6.6**) are fewer
and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
`unmixLinear(-0.35, -0.6)`. Size: cutoff **C = 0.4** on *both* `glaciers.json` (ice,
...
### packwiz `side` (as of 0.5.2–0.6.6)
