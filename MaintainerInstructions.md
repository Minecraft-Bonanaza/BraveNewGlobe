...
Current pack version is **0.6.11** (`pack.toml` matches; the field lagged at `0.6.7`
through 0.6.8–0.6.10). **Fresh world required** for players upgrading from any version
before **0.6**. 0.6.1–0.6.11 do not need a new world if they are already on 0.6.

## Height-patched Big Globe (as of 0.6, still in 0.6.11)
...
**0.6.11**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
### packwiz `side` (as of 0.5.2–0.6.11)
...
patched Big Globe jar are `side = "both"`.** Do not mark them client-only. **C2ME is not
in the pack** (removed 0.6.10). **Drive-By-Wire With Sable is not in the pack.**
