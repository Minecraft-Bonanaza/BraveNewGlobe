...
- [`MODLIST.md`](MODLIST.md) — human-readable grouped list of every mod (currently **136**).
...
0.7.18 is **Create: Villager Commerce** `1.0.1` (CurseForge 1557618; no jar rebuild, no
worldgen; relaunch). Another **unversioned** add after 0.7.18 is **SPECTRUM.md** (docs
only; no jar / worldgen). **0.7.17** / **0.7.18** (same author commit `5b385da`) swap
the coin economy to **Create: Numismatics** plus Delivery Required and the
shop/stock-market stack (no jar rebuild, no worldgen; relaunch). An **unversioned**
add after 0.7.16 still includes **Create: Linear Bearing** `1.2.6`, **VS / Sable Hose
Connectors** `0.1.8`, and **MODLIST.md**. **0.7.16** adds **Better Combat**
...
**Create: Villager Commerce** (`pack/mods/create-villager-commerce.pw.toml`) is CurseForge
`1.0.1` (project 1557618 / file 8183116). Keep `side = "both"`. Required dep already in
the pack: Create. Optional Numismatics is already present (preset trades pay in coins).
Do not invent a pack version for it — `pack.toml` stayed **0.7.18**.

**`MODLIST.md`** lists all 136 mods grouped by purpose. Update it in the same commit as
...
> currently **136**). If the mod joins or leaves a career path, update [`SPECTRUM.md`](SPECTRUM.md)
...
Market / Utils), Create Aeronautics: Delivery Required, Create: Villager Commerce, and the
