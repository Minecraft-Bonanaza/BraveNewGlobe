...
Current pack version is **0.7.16** (`pack.toml` matches). **0.7.16** adds **Better Combat**
...
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.7.16.
...
- Giant towers: `keep_kayra` / `infested_temple` / `kisegi_sanctuary` start **0** +
  ...
  current. Compat history is in CHANGELOG [0.7.0]–[0.7.16].
...
**0.7.16**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
### packwiz `side` (as of 0.5.2–0.7.16)
...
Dungeons Arise, Better Combat, and the patched Big Globe jar are `side = "both"`.** Do not mark them
