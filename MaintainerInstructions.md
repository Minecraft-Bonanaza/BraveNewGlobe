...
Village, glacier, overworld-depth, spawn-clamp, and chunk-performance behavior for the current pack
is documented in [Notes.md](Notes.md). After editing a zip under `pack/datapacks/`, run
`packwiz refresh` before committing.

Changelog / commit titles are **0.6.10**. `pack.toml` version string is still **0.6.7**
(0.6.8–0.6.10 re-indexed the hash but did not bump the version field). **Fresh world
required** for players upgrading from any version before **0.6**. 0.6.1–0.6.10 do not
need a new world if they are already on 0.6.

## Height-patched Big Globe (as of 0.6, still in 0.6.10)
...
**0.6.10**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
### packwiz `side` (as of 0.5.2–0.6.10)
...
**Creating Space, Vertigo, In Control!, and the patched Big Globe jar are `side = "both"`.**
Do not mark them client-only. **C2ME is not in the pack** (removed 0.6.10).
