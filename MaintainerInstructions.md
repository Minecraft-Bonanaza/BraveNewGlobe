...
Current pack version is **0.7.8** (`pack.toml` matches). **0.7.2–0.7.8** are datapack-only
...
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.7.8.
...
- Giant towers: `keep_kayra` / `infested_temple` / `kisegi_sanctuary` start **−15** +
  `WORLD_SURFACE_WG` + **`beard_box`**, gated to `#bigglobe:land`. −15 stays above
  BG's `surface−16` cave threshold so the biome check still matches. Do **not** sink
  them to −16 or deeper (0.7.2's −45/−25 produced **zero spawns**). Do **not**
  restore infested/kisegi to −45/−25. Compat history is in CHANGELOG [0.7.0]–[0.7.8].
...
**0.7.8**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
### packwiz `side` (as of 0.5.2–0.7.8)
