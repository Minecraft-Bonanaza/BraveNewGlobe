...
- [`MODLIST.md`](MODLIST.md) — human-readable grouped list of every mod (currently **129**).
...
0.7.16: **Create: Linear Bearing** `1.2.6`, **VS / Sable Hose Connectors** `0.1.8`, and
**MODLIST.md** (no jar rebuild, no worldgen; relaunch). **0.7.16** adds **Better Combat**
...
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.7.16 or the
unversioned add after.
...
- Giant towers: `keep_kayra` / `infested_temple` / `kisegi_sanctuary` start **0** +
  ...
  current. Compat history is in CHANGELOG [0.7.0]–[0.7.16] and the unversioned add after.
...
**0.7.16** / the unversioned add after) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
add VS for this mod. No committed config for either new mod.

**`MODLIST.md`** lists all 129 mods grouped by purpose. Update it in the same commit as
...
> currently **129**). packwiz metafiles remain the source of truth.
...
### packwiz `side` (as of 0.5.2–0.7.16 / unversioned after)
...
patched Big Globe jar are `side = "both"`.** Do not mark them
