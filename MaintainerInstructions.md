...
Current pack version is **0.8.5** (`pack.toml` matches). **0.8.5** adds **Ponder
...
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.8.5.
...
## When Dungeons Arise (as of 0.7.15, still in 0.8.5)
...
- Giant towers: `keep_kayra` / `infested_temple` / `kisegi_sanctuary` start **0** +
  ...
  current. Compat history is in CHANGELOG [0.7.0]–[0.8.5].
...
**0.8.5**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
### packwiz `side` (as of 0.5.2–0.8.5)
