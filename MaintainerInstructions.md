...
Current pack version is **0.7.9** (`pack.toml` matches). **0.7.2–0.7.9** are datapack-only
...
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.7.9.
...
- WDA `major`/`minor` structure sets are **emptied** (do not restore them — structures would
  double-place).
- 24 of 38 structures live in four `stattinkerer` sets: `:sky` (**5**, spacing 64/**44**),
  `:sea` (4, 48/42), `:common` (11, 32/28), `:large_dungeon` (**4**, 64/56). 14 are in no
  set (removed). Do **not** put `heavenly_challenger` back in `large_dungeon` (moved to
  sky in 0.7.2). `mechanical_nest` stays in `large_dungeon` even though it flies at
  Y 700–750.
...
- Giant towers: `keep_kayra` / `infested_temple` / `kisegi_sanctuary` start **0** +
  `WORLD_SURFACE_WG` + **`beard_box`**, gated to `#bigglobe:land`. `start 0` is the
  proven 0.7.0/0.7.1/0.7.7 anchor — a full 16-block margin above BG's `surface−16`
  cave threshold so `#land` still matches. Do **not** sink them to −16 or deeper
  (0.7.2's −45/−25 produced **zero spawns**). Do **not** restore infested/kisegi to
  −45/−25. Do **not** re-apply 0.7.8's −15 (1-block margin, fragile). Compat
  history is in CHANGELOG [0.7.0]–[0.7.9].
...
**12 chunks** (lives in the WDA zip). A set allows only one exclusion_zone; the chain
is `sky → large_dungeon → villages` (no cycle). Do **not** shrink the dungeon buffer
back to 6.

**Ocean glaciers** (`pack/datapacks/bigglobe_less_glacier.zip`, as of **0.6.7**, still in
**0.7.9**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
### packwiz `side` (as of 0.5.2–0.7.9)
