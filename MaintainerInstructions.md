...
- [`MODLIST.md`](MODLIST.md) — human-readable grouped list of every mod (currently **141**).
...
Current pack version is **0.8.4** (`pack.toml` matches). **0.8.4** adds **LDLib2**
`2.2.37` and restores **owo-lib** `0.12.15.5-beta.1` (no jar rebuild, no worldgen;
relaunch). **PonderJS is not in the pack** (no 1.21.1 NeoForge build). **0.8.3**
...
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.8.4.
...
## When Dungeons Arise (as of 0.7.15, still in 0.8.4)
...
- Giant towers: `keep_kayra` / `infested_temple` / `kisegi_sanctuary` start **0** +
  ...
  current. Compat history is in CHANGELOG [0.7.0]–[0.8.4].
...
**0.8.4**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
`side = "both"`. **PonderJS is not in the pack** (no 1.21.1 NeoForge build).

**`MODLIST.md`** lists all 141 mods grouped by purpose. Update it in the same commit as
...
in-person / physical freight). Do not write quests that require PonderJS.
...
> currently **141**). If the mod joins or leaves a career path, update [`SPECTRUM.md`](SPECTRUM.md)
...
### packwiz `side` (as of 0.5.2–0.8.4)
...
FTB Quests / FTB Library / FTB Teams, LDLib2, owo-lib, and the
...
in 0.8.4.** **PonderJS is not in the pack.**
