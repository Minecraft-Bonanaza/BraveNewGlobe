...
Current pack version is **0.7.6** (`pack.toml` matches). **0.7.2–0.7.6** are datapack-only
...
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.7.6.
...
- Current underground depths (all `#bigglobe:underground`, `bury`, in `common`):
  **scorched_mines −150**, **plague_asylum −300**, **foundry −520**. Do **not** drop
  foundry to −560/−540 (floor clip vs −608) or −100. Keep it in core/molten (−592..−496).
- Giant towers: `keep_kayra` start **0**; **infested_temple −45**; **kisegi_sanctuary −25**
  (still `WORLD_SURFACE_WG` + `beard_thin`). Do **not** restore those two to 0 — they
  floated. Compat history is in CHANGELOG [0.7.0]–[0.7.6].
...
**0.7.6**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
### packwiz `side` (as of 0.5.2–0.7.6)
