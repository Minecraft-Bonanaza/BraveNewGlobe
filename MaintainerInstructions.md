...
Current pack version in `pack.toml` is **0.6.4**. **Fresh world required** for players upgrading
from any version before **0.6**. 0.6.1–0.6.4 are datapack-only (ocean glaciers) and do not
need a new world if they are already on 0.6.

## Height-patched Big Globe (as of 0.6, still in 0.6.4)
...
**Ocean glaciers** (`pack/datapacks/bigglobe_less_glacier.zip`, as of **0.6.4**) drive
fill off proven `glacier_crack_threshold`:
`hard_distance <= min(1.4 × (glacier_crack_threshold − 0.60), 0.75)`. Do **not** re-add
`glacier_field.json` — 0.6.1's new noise loaded but never cleared the `0.5` cutoff, so
ice fill was zero everywhere. Living knobs: the `0.60` cutoff is the **size dial**
(higher → smaller / deeper-core only; lower toward 0.25 → larger / continent); the
`0.75` cap keeps the main mass as broken floes. Details in [Notes.md](Notes.md). Only
new cold-ocean chunks pick up glacier changes.
...
### packwiz `side` (as of 0.5.2–0.6.4)
