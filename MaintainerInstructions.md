...
Current pack version is **0.7.1** (`pack.toml` matches). **0.7.1** is a datapack-only
hotfix (no jar rebuild; relaunch). **Fresh world required** for players upgrading from
**0.6.x** (ceiling +896 → +1024; or regenerate the top + clear DH) and from any version
before **0.6** (floor −608).
...
**0.7.1**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
### packwiz `side` (as of 0.5.2–0.7.1)
