...
Village, glacier, and overworld-depth behavior for the current pack is documented in
[Notes.md](Notes.md). After editing a zip under `pack/datapacks/`, run `packwiz refresh`
before committing.
...
**0.6-beta performance work is parked on branch `0.6-beta`** (Vertigo +
`bigglobe_shallow_overworld.zip`). Do not re-add those to main until Connector
networking-mixin + Distant Horizons testing is done. `pack.toml` version is still
`0.6-beta` even though those files are gone; bump it when you next version the pack.
...
**Creating Space is `side = "both"`** (0.6). Do not mark it client-only; planet dimensions
and rockets need to exist on the server.
