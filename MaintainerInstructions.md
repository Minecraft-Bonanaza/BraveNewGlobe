...
Current pack version in `pack.toml` is **0.6.2**. **Fresh world required** for players upgrading
from any version before **0.6**. 0.6.1 / 0.6.2 are datapack-only (ocean glaciers) and do not
need a new world if they are already on 0.6.

## Height-patched Big Globe (as of 0.6, still in 0.6.2)
...
**Ocean glaciers** (`pack/datapacks/bigglobe_less_glacier.zip`, as of **0.6.2**) drive
fill off proven `glacier_crack_threshold`:
`hard_distance <= 1.4 × (glacier_crack_threshold − 0.25)`. Do **not** re-add
`glacier_field.json` — 0.6.1's new noise loaded but never cleared the `0.5` cutoff, so
ice fill was zero everywhere. Living knobs (the `0.25` cutoff, `1.4` multiplier, Voronoi
variation 40) are in [Notes.md](Notes.md). Only new cold-ocean chunks pick up glacier
changes.
...
### packwiz `side` (as of 0.5.2–0.6.2)
