...
- [`MODLIST.md`](MODLIST.md) — human-readable grouped list of every mod (currently **145**).
  Update it when adding or removing mods; packwiz metafiles remain the source of truth.
- [`SPECTRUM.md`](SPECTRUM.md) — Special Interest Group (SIG) framework for the
  **FTB Quests** book. **FTB Quests is in the pack.** Update this file when a mod
  creates, joins, or is excluded from a career path.
- [`QUESTS.md`](QUESTS.md) — locked 15-line guidance roadmap. **Create Core** is
  **12 purpose-driven quests** (0.8.7). Committed `.snbt` under `pack/config/ftbquests/`
  is the source of truth (`bigGlobeAero/build_ftbquests.py`). IDs are SHA-1 of
  stable keys (0.8.1); changing keys (as 0.8.7 did) is a content rewrite.
...
Current pack version is **0.8.7** (`pack.toml` matches). **0.8.7** reworks
**Create Core** to **12 purpose-driven quests** (use/produce + automate,
bootstrap-kit rewards). Same **145** mods. **0.8.6** re-refreshes
the stale `pack.toml` `[index]` hash left by 0.8.5 (packwiz **"index hash file
invalid"** — `index.toml` was updated, the hash field was not). No mod changes.
**0.8.5** adds **Ponder
for KubeJS** (`ponderjs` 1.21.1-2.4.0, slug **`ponder`**) plus **KubeJS**
`2101.7.2`, **Rhino** `2101.2.8`, and **Better Advanced Tooltips** `2101.1.0` —
required by **Delivery Required** (crash on load without them). **0.8.4** adds
**LDLib2** `2.2.37` and restores **owo-lib** `0.12.15.5-beta.1`. **0.8.3**
self-hosts **Create: Villager Commerce** in `bundled-jars/`. **0.8.2** re-points
**Create: Linear Bearing** to Modrinth `1.3.5`. **0.8.1** / **0.8.0** / **0.7.19**
add **FTB Quests** (Create Core shipped). **Fresh world required** for players
upgrading from **0.6.x** (ceiling +896 → +1024; or regenerate the top + clear DH) and
from any version before **0.6** (floor −608). **No fresh world** for 0.7.1→0.8.7.
...
## When Dungeons Arise (as of 0.7.15, still in 0.8.7)
...
- Giant towers: `keep_kayra` / `infested_temple` / `kisegi_sanctuary` start **0** +
  ...
  current. Compat history is in CHANGELOG [0.7.0]–[0.8.7].
...
**0.8.7**) are fewer and smaller solid sheets. Frequency: `glacier_crack_threshold` temp bar
...
**Create: Linear Bearing** (`pack/mods/linear-bearing.pw.toml`) is Modrinth `1.3.5`
(`ZTwCjE7O` / `8SJdnoXS`) as of **0.8.2** (was CurseForge `1.2.6`; CF
`allowModDistribution:false`). Keep `side = "both"`. Deps already in the pack: Create,
Create Aeronautics (`aeronautics_bundled` 1.3.1), Sable `2.0.5`. Do not drop Sable.
Do not `packwiz curseforge add` it back over the Modrinth metafile.
...
Numismatic Overhaul, the old Numismatic Bounties, and owo-lib**. Do not re-add
Overhaul or the old Bounties. **owo-lib was restored in 0.8.4** — do not drop it
with the old Overhaul-only reasoning.
...
**Create: Villager Commerce** (`pack/mods/create-villager-commerce.pw.toml`) is
self-hosted as of **0.8.3**: `bundled-jars/createvillagercommerce-1.0.1.jar`,
raw-GitHub URL, sha256-pinned. CF distribution is disabled and it is not on
Modrinth — do not switch the metafile back to `metadata:curseforge`. Keep
`side = "both"`. Required dep already in the pack: Create. Optional Numismatics
is already present (preset trades pay in coins).

**FTB Quests** (`pack/mods/ftb-quests-forge.pw.toml`) is CurseForge `2101.1.34`
(project 289412) with **FTB Library** `2101.1.35` (404465) and **FTB Teams**
`2101.1.11` (404468). Keep `side = "both"`. Official FTB mods are not on
Modrinth. Committed quest `.snbt` is the source of truth — regenerate from
`bigGlobeAero/build_ftbquests.py`. IDs are SHA-1 of stable keys (0.8.1). Same-key
regens are progress-safe; changing keys (as **0.8.7** did for Create Core) is a
content rewrite. Do not overwrite via in-game edit mode.

**LDLib2** (`pack/mods/ldlib.pw.toml`) is CurseForge `2.2.37` (project 626676 /
file 8724783). Keep `side = "both"`. **owo-lib** (`pack/mods/owo-lib.pw.toml`)
is CurseForge `0.12.15.5-beta.1` (project 532610 / file 6785734). Keep
`side = "both"`.

**Ponder for KubeJS** (`pack/mods/ponder.pw.toml`) is CurseForge `1.21.1-2.4.0`
(project 622888 / file 7436813, slug **`ponder`**, modId `ponderjs`). Keep
`side = "both"`. Also required: **KubeJS** `2101.7.2` (238086 / 8715199),
**Rhino** `2101.2.8` (416294 / 8463898), **Better Advanced Tooltips** `2101.1.0`
(1637623 / 8576077). Do not `packwiz curseforge add ponderjs` (wrong slug).
Do not drop this chain — **Delivery Required** crashes without it.

**`MODLIST.md`** lists all 145 mods grouped by purpose. Update it in the same commit as
`packwiz curseforge add` / removals so the count and groupings stay current. packwiz
metafiles remain the source of truth.

**`SPECTRUM.md`** is the SIG catalog; **`QUESTS.md`** is the locked 15-line
roadmap. **FTB Quests is in the pack** (Create Core is **12 purpose-driven quests**
as of 0.8.7). When adding a mod,
ask: does it create a baseline → specialized curve? Which SIG owns it?
If none, is it world / fauna / QoL instead? Update `SPECTRUM.md` / `QUESTS.md`
in the same commit. Do not make **Create** itself a SIG. Do not quest
player-minted coins or remote / mailbox checkout (Commerce / Logistics stay
in-person / physical freight). Optional: point players at Ponder scenes. Do
not make KubeJS scripting a quest line.
...
Sea Myths) are still the 0.4 CurseForge-distribution workaround. **Create: Villager
Commerce** joined that set in **0.8.3**.
...
> ⚠️ Always `packwiz refresh` before committing, then commit **both** `index.toml`
> **and** `pack.toml`. Refresh rewrites the index *and* the `[index]` hash in
> `pack.toml`. **0.8.5** committed the new `index.toml` with a stale hash field —
> packwiz then reported **"index hash file invalid"** until **0.8.6**. Skipping
> refresh (or committing only one of those two files) breaks every player's launch.
>
> Also update [`MODLIST.md`](MODLIST.md) in the same commit when you add or remove a mod (count is
> currently **145**). If the mod joins or leaves a career path, update [`SPECTRUM.md`](SPECTRUM.md)
> and [`QUESTS.md`](QUESTS.md) too. packwiz metafiles remain the source of truth.
...
- After any real change under `pack/`, run `packwiz refresh` so `index.toml` **and**
  `pack.toml`'s `[index]` hash match the bytes you are about to commit. Commit both
  files. **0.8.5** forgot the hash field (fixed in **0.8.6**).
...
### packwiz `side` (as of 0.5.2–0.8.7)
...
KubeJS / Rhino / Better Advanced Tooltips, and the
patched Big Globe jar are
`side = "both"`.** Do not mark them client-only. **C2ME is not in the pack** (removed 0.6.10).
**Drive-By-Wire With Sable is not in the pack.** **Numismatic Overhaul and the old
Numismatic Bounties are not in the pack** (removed 0.7.17). **owo-lib was restored
