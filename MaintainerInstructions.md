...
## Current pack
**0.9.5** / **153** mods. Do not `packwiz update` Big Globe (pinned patched jar, no `[update]`
block). Keep Simply More on Modrinth release **`1.2.3`** (`simplymore.pw.toml`; skip
`1.3.0_alphaN`; do not `packwiz curseforge add` it — CF distribution is disabled). LootJS /
Simply Swords / Simply More / Fzzy Config / Simply Tooltips / Incendium / Born in Chaos are
`side = "both"`. **YUNG's Better Nether Fortresses is `side = "server"`** (PrismLauncher default
`--side client` skips it). Born in Chaos overworld structure compat is
`pack/datapacks/bigglobe_borninchaos.zip`.

## FTB Quests
The shipped book is **15 chapters / 230 quests** (0.9.0; unchanged through 0.9.5). Design rules:
...
- Simply Swords / Simply More are loot *candidates* (0.9.2); Incendium / Born in Chaos drops
  are too (0.9.4). Add them to LOOT.md before wiring.
