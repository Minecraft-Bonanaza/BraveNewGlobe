...
Current pack version in `pack.toml` is **0.6.7**. **Fresh world required** for players upgrading
from any version before **0.6**. 0.6.1–0.6.7 are datapack-only (ocean glaciers) and do not
need a new world if they are already on 0.6.

## Height-patched Big Globe (as of 0.6, still in 0.6.7)
...
**Ocean glaciers** (`pack/datapacks/bigglobe_less_glacier.zip`, as of **0.6.7**) are fewer
...
### packwiz `side` (as of 0.5.2–0.6.7)
