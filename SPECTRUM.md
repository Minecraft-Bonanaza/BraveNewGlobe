...
in 0.8.1). Other lines are still pending. **0.8.6** is a packwiz hash-only
fix (no quest changes). **PonderJS is in** (0.8.5; required by
Delivery Required). Optional: point players at in-world Ponder scenes. Do not
make KubeJS scripting a quest line. Quest `.snbt` under `pack/config/ftbquests/`
is the source of truth — regenerate from `bigGlobeAero/build_ftbquests.py`
rather than overwriting via in-game edit mode.
...
**PonderJS is in** — optional scene pointers are fine; do not add a KubeJS scripting line.
