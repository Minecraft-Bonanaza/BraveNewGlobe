...
**[QUESTS.md](QUESTS.md)**. **Create Core** has shipped (0.8.0; deterministic IDs
in 0.8.1). Other lines are still pending. **PonderJS is not in the pack** (no
1.21.1 NeoForge build). Quest `.snbt` under `pack/config/ftbquests/` is the
source of truth — regenerate from `bigGlobeAero/build_ftbquests.py` rather than
overwriting via in-game edit mode.
...
| **Libraries / APIs** | GeckoLib, Cloth Config, Villager API, LDLib2, owo-lib, … | Plumbing |
...
- **Create Core** is the shared industrial onboarding line (see [QUESTS.md](QUESTS.md)), not a SIG.  
- **PonderJS is not in the pack.** Do not write quests that assume in-world Ponder scenes.

### Libraries

All Shared Libraries & APIs in [MODLIST.md](MODLIST.md) — never SIG content. Includes
**LDLib2** and **owo-lib** (restored 0.8.4).
...
behind another). **Create Core** is in-game as of 0.8.0.
...
(0.8.1) — regenerating a chapter does not reset player progress.
...
**PonderJS is not in the pack** — do not add quests that require it.
