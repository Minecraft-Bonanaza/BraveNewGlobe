...
## Overworld depth — height-patched Big Globe (as of 0.6, still in 0.6.6)
`pack.toml` version is **0.6.6**. Overworld bounds are **floor −608**, **ceiling +896**, sea
...
= **−608** and Distant Horizons LODs align with real terrain. **0.6.1–0.6.6 do not change
...
## Chunk performance — C2ME + Vertigo (as of 0.5.7, still in 0.6.6)
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.6)
...
- **0.6.6 (current)** — **fewer + smaller**. Colder temperature bar, and cutoff **C = 0.4**
  applied to *both* ice and biome so the footprints stay matched.

Current rules:

- **`glacier_crack_threshold.json`** — temperature bar
  `unmixLinear(-0.35, -0.6, temperature_at_sea_level)` (was `-0.25 / -0.5`), still min'd
  with the depth term. Glacier oceans only appear where it is colder → **rarer**.
...
- **Frequency:** the temp bar. Colder (`-0.35 / -0.6` vs `-0.25 / -0.5`) → rarer glacier
  oceans.
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.6.6)
