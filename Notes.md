...
## Overworld depth — height-patched Big Globe (as of 0.6, still in 0.6.2)
`pack.toml` version is **0.6.2**. Overworld bounds are **floor −608**, **ceiling +896**, sea
...
= **−608** and Distant Horizons LODs align with real terrain. **0.6.1 and 0.6.2 do not change
height** (glacier datapack only) — players already on a 0.6 world can keep it.
...
## Chunk performance — C2ME + Vertigo (as of 0.5.7, still in 0.6.2)
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.2)
`pack/datapacks/bigglobe_less_glacier.zip` retunes the glacier *feature* only. The glacier
*biome* extent and Aquamirae's Sea of Shivers are not overridden.

0.6.1 (**v1**) tried a new `glacier_field` noise (`2.0 × (glacier_field − 0.5)`). The noise
loaded (no log error) but never cleared the `0.5` cutoff, so fill was ≤ 0 everywhere →
**zero ice**. 0.6.2 (**v2**) drops that noise and drives fill off the **existing, proven
`glacier_crack_threshold`** — the same field the pre-0.6 datapack already used to place ice.

Current rules:

- **`glaciers.json` dispatcher** — a Voronoi cell fills when
  `hard_distance <= 1.4 × (glacier_crack_threshold − 0.25)`. Threshold ≤ 0.25 is **open
  water**; rising threshold grows floes; deep/cold cores (threshold ≈ 1) fully fill and
  merge into a **sheet**. Concentration and thinning come from `crack_threshold`'s natural
  cold/depth gradient (dense over deep cold water, fading toward shallows).
...
- **`glacier_cell`** Voronoi `distance` stays **48** (from 0.5); `variation` stays **40**
  (from 0.6.1) so floes scatter instead of lining up on a grid.
- There is **no `glacier_field.json`** anymore.

Tunable knobs:

- Coverage: the `0.25` cutoff in `glaciers.json` (raise → less ice).
- Sheet size: the `1.4` fill multiplier (raise → bigger sheets).

Caveats:

- Guaranteed to place ice (reuses the proven field). The look is **"dense core → thinning
  to shallows"**, not discrete island sheets. Peak-noise modulation can come back later —
  v1 confirmed a custom column value loads.
- Only **newly generated** cold-ocean chunks pick it up. No jar or world reset. Explore
  fresh ocean (or delete those chunks) to see the new pattern. 0.6.1 chunks that generated
  with no ice stay empty until regenerated.

## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.6.2)
