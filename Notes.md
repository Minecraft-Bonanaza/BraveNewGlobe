...
## Overworld depth — height-patched Big Globe (as of 0.7.0, still in 0.8.5)
...
ore curves are unchanged from 0.6. **0.7.1–0.8.5 did not rebuild the jar** (0.7.1–0.7.15
...
- From **0.7.1 → 0.8.5** — **no** fresh world for height. 0.7.1–0.7.15 are
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0–0.8.5):
...
**0.7.2–0.8.5** did not change the 24/38 split (same 14 removed). 0.7.2–0.7.15 retuned
placement / heights / set membership (nest split in 0.7.12). **0.7.16–0.8.5** did not
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.8.5)
...
**0.6.8–0.8.5 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.8.5)
...
**0.7.16**, in the same commit as **`MODLIST.md`**. Current `pack.toml` is **0.8.5**.
...
**`MODLIST.md`** is a human-readable grouped list of all **145** mods (as of 0.8.5;
...
`pack.toml` is **0.8.5**. `side = "both"`. No worldgen. **No fresh world.**
...
[`MODLIST.md`](MODLIST.md) (count is **145** as of 0.8.5).
