...
## Overworld depth — height-patched Big Globe (as of 0.6, still in 0.6.4)
`pack.toml` version is **0.6.4**. Overworld bounds are **floor −608**, **ceiling +896**, sea
...
= **−608** and Distant Horizons LODs align with real terrain. **0.6.1–0.6.4 do not change
...
## Chunk performance — C2ME + Vertigo (as of 0.5.7, still in 0.6.4)
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.4)
...
- **0.6.4 (current)** — raised the coverage cutoff `0.25 → 0.60` so ice only forms over the
  **deep/cold core**. In-between of v1 (off) and v3 (continent).

Current rules:

- **`glaciers.json` dispatcher** — a Voronoi cell fills when
  `hard_distance <= min(1.4 × (glacier_crack_threshold − 0.60), 0.75)`. Threshold ≤ 0.60
  is **open water**; rising threshold grows floes; the `0.75` cap stops cells from fully
  merging, so the core is **large broken floes**, not one seamless sheet.
...
- **Size / footprint dial:** the `0.60` cutoff in `glaciers.json`. Higher → smaller
  footprint (deeper core only). Lower (toward 0.25) → larger, back toward continent-sized.
  ~1.0+ → essentially off (like v1).
- **Max sheet size:** the `0.75` cap (lower → smaller / more-broken main sheet).
- The `1.4` multiplier is still in the formula; the cutoff is the intended size dial.

Caveats:

- Guaranteed to place ice (reuses the proven `glacier_crack_threshold` field). The look is
  **"broken floes over the deep/cold core"**, not a continent and not discrete island sheets.
- Only **newly generated** cold-ocean chunks pick it up. No jar or world reset. Explore
  ...
  with no ice stay empty until regenerated. 0.6.2 / 0.6.3 chunks keep their larger sheets
  until regenerated.

## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.6.4)
