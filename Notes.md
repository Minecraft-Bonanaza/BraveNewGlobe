...
## Overworld depth — height-patched Big Globe (as of 0.7.0, still in 0.8.7)
...
ore curves are unchanged from 0.6. **0.7.1–0.8.7 did not rebuild the jar** (0.7.1–0.7.15
...
**0.8.6** is the hash-only fix; **0.8.7** is the Create Core quest rework).
...
- From **0.7.1 → 0.8.7** — **no** fresh world for height. 0.7.1–0.7.15 are
  datapack-only WDA placement retunes; later versions add combat / economy /
  quests / libraries (no worldgen). **0.8.6** is hash-only. **0.8.7** rewrites
  Create Core (quest book only). Relaunch.
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0–0.8.7):
...
**0.7.2–0.8.7** did not change the 24/38 split (same 14 removed). 0.7.2–0.7.15 retuned
placement / heights / set membership (nest split in 0.7.12). **0.7.16–0.8.7** did not
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.8.7)
...
**0.6.8–0.8.7 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.8.7)
...
**0.7.16**, in the same commit as **`MODLIST.md`**. Current `pack.toml` is **0.8.7**.
...
**`MODLIST.md`** is a human-readable grouped list of all **145** mods (as of 0.8.7;
...
`pack.toml` is **0.8.7**. `side = "both"`. No worldgen. **No fresh world.**
...
[`MODLIST.md`](MODLIST.md) (count is **145** as of 0.8.7).
...
**FTB Quests is in the pack** (0.7.19+). **Create Core** shipped in 0.8.0 and
was reworked in **0.8.7** to **12 purpose-driven quests**. Keep SIG IDs stable
and update `SPECTRUM.md` / `QUESTS.md` in the same commit as quest `.snbt` changes.
...
## FTB Quests (as of 0.7.19 / 0.8.0 / 0.8.1 / 0.8.7)
...
**0.8.0** shipped the first chapter **Create Core**
(`pack/config/ftbquests/quests/chapters/create_core.snbt`) as 15 "craft the block"
guidance quests (Andesite Alloy → kinetics → casings / machines → brass →
Precision Mechanism → Mechanical Mastery). The 15-line roadmap is
**[QUESTS.md](QUESTS.md)**. Generated by `bigGlobeAero/build_ftbquests.py`; the
committed `.snbt` is the **source of truth** — regenerate from the script rather
than overwriting via in-game edit mode.

**0.8.1** derives all FTB Quests IDs (chapter / quest / task / reward) from
**stable keys** via SHA-1 instead of random hex. Regenerating a chapter with the
**same keys** yields identical IDs, so those re-runs are **progress-safe**. Create
Core's IDs were regenerated **once** to those stable values — anyone who completed
Create Core on **0.8.0** (random IDs) saw those quests as new after 0.8.1.

**0.8.7** rebuilds Create Core (15 → **12 quests**) as a **purpose/automation**
chapter (CAB style): each quest has you **use a machine to produce an output or
do a task**. Item tasks ask for signature outputs (crushed raw ore, iron sheets,
brass, precision mechanism); ambiguous steps (fan, saw, deployer, belts, fluids,
capstone automation) use player-attested **checkmark** tasks. Capstone is a
**passive Andesite Alloy line**. Rewards are scaling XP plus **bootstrap-kit**
items (casings, sheets, brass, precision mechanisms). This is a **content rewrite**
(new keys / fewer quests), not a same-key regen — expect mixed progress if you
completed the 0.8.0 / 0.8.1 chapter. The generator gained checkmark tasks,
item-count tasks, and item rewards.

No worldgen. **No fresh world.** Relaunch so packwiz pulls the updated quest file.
