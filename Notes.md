...
## Overworld depth — height-patched Big Globe (as of 0.7.0, still in 0.7.6)
Current pack version is **0.7.6** (`pack.toml` matches). Overworld bounds are **floor −608**,
...
ore curves are unchanged from 0.6. **0.7.1–0.7.6 did not rebuild the jar** (datapack-only
...
- From **0.7.1 → 0.7.6** — **no** fresh world for height. Datapack-only WDA placement
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0–0.7.6):
...
## When Dungeons Arise (as of 0.7.6)
...
| `stattinkerer:large_dungeon` | 4 | 64 / 56 | massive dungeons (`keep_kayra`, seated `infested_temple` / `kisegi_sanctuary`, flying `mechanical_nest`); 12-chunk exclusion vs villages |
...
Compat mismatches vs WDA 2.1.68 + this shallow world (author 0.7.0, depths retuned 0.7.2–0.7.6):
...
- **`foundry`** — was Y 900 in `#bigglobe:nether` → overworld underground forge
  ...
  (core/molten) → 0.7.5 **−540** (floor clearance) → **0.7.6 −520** (current: 88 above
  the −608 floor, 56 above the lava sea at −576, still inside core/molten −592..−496).
  Do **not** drop it back to −560/−540 (a downward piece can clip the world floor) or
  to −100.
...
- Giant towers: **`keep_kayra`** stays `start_height 0` + projection + `beard_thin`.
  **0.7.2** seated **`infested_temple` 0 → −45** and **`kisegi_sanctuary` 0 → −25**
  (jigsaw start-anchor sits above the build's base, so `start_height 0` left them
  floating). Both still `WORLD_SURFACE_WG` + `beard_thin`. On steep slopes a residual
  gap can remain; `beard_box` is the stronger option if needed. **`mechanical_nest`**
...
**0.7.2–0.7.6** did not change the 24/38 split (same 14 removed). They only retuned
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.7.6)
...
**0.6.8–0.7.6 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.7.6)
