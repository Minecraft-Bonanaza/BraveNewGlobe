...
## Overworld depth — height-patched Big Globe (as of 0.7.0)
Current pack version is **0.7.0** (`pack.toml` matches). Overworld bounds are **floor −608**,
**ceiling +1024**, sea level 0. That height lives inside a patched Big Globe 5.3.2 jar
(`bundled-jars/bigglobe-5.3.2-mc1.21.1-shallow608.jar`, `pack/mods/big-globe.pw.toml`).
The filename still says `shallow608` because **608 is the floor**; 0.7.0 rebuilt the jar
(sha256 `df683d31…`) to restore the stock **+1024** ceiling for airships / aerial structures
while keeping the deep-floor disk. Underground layers + ore curves are unchanged from 0.6.
`cloud_height` is already 1024. Distant Horizons anchors to generator `min_y` = −608 and
now renders LODs up to +1024 — no offset.
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0):
...
|---|---|---|---|
| `stattinkerer:sky` | 4 | 64 / 56 | aerial airship targets |
| `stattinkerer:sea` | 4 | 48 / 42 | aquatic ships at sea level |
| `stattinkerer:common` | 11 | 32 / 28 | smaller land / mixed builds |
| `stattinkerer:large_dungeon` | 5 | 64 / 56 | massive dungeons + flying `heavenly_challenger` |

Weights are still WDA originals; spacings pending further tuning. Groupings live in
`bigGlobeAero/patch_wda_compat.py`.
...
Compat mismatches vs WDA 2.1.68 + this shallow world (author 0.7.0):

- **`foundry`** — was Y 900 in `#bigglobe:nether` → overworld underground forge (Y −100,
  `#bigglobe:underground`, `bury`).
...
- **`scorched_mines`** re-gated from `bigglobe:molten_cave` (only exists ~Y −496+ here) to
  `#bigglobe:underground`, moved to **Y −200**.
...
- Giant towers (`keep_kayra`, `infested_temple`, `kisegi_sanctuary`) grounded
  (`start_height 0` + projection + `beard_thin`). **`mechanical_nest`** flew to Y 700–750.
  Those four re-gated to `#bigglobe:land`.
...
## Villages (CTOV only, as of 0.5 / 0.5.1; buffers as of 0.7.0)
...
- **Village buffers (0.7.0):** `bigglobe_ctov:pillager_outposts` and
  `stattinkerer:large_dungeon` each have an `exclusion_zone` vs `bigglobe_ctov:villages` at
  **6 chunks**, so outposts and large WDA dungeons keep clear of villages. A structure set
  allows only one exclusion_zone, so it is applied on the avoiders (villages stay the
  priority). Patched via `bigGlobeAero/patch_ctov_compat.py`.

`pack/config/bettervillage_1.properties` is leftover from Better Villages and is unused.

## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.7.0)
...
**0.6.8–0.7.0 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.7.0)
