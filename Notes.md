...
## Shallow overworld (`bigglobe_shallow_overworld`, as of 0.6-beta)
`pack/datapacks/bigglobe_shallow_overworld.zip` compresses Big Globe's overworld vertical
extent (~55% less underground storage / per-chunk gen). **Fresh world required** — this
changes dimension bounds.

Current bounds (verified against Big Globe 5.3.2):

- Floor `-1024 → -464`, ceiling `+1024 → +896`, sea level still **0**.
- Dimension type height / logical height **1360** (`min_y` -464).
- Deep tiers kept but compacted: core **80** thick (`-448…-368`), deep dark **64**
  (`-352…-288`), lava sea at `-432`, base stone trimmed 112 → 16.
- Surface, caves, ores (surface-relative) and biomes are unchanged.
- Overrides 5 Big Globe files: `dimension_type/overworld`, `world_preset/bigglobe` generator
  height, `bigglobe_world_trait_impl/overworld` tier constants, `the_core` molten gradient,
  `test_core` biome interface.

Do not reuse a world generated before 0.6-beta.

## Vertigo (as of 0.6-beta)
**Vertigo** `1.2.4` (Modrinth `4LzgJp1j`, `pack/mods/vertigo.pw.toml`) skips sending empty
vertical chunk sections. Fabric mod; Fabric API is provided by **Forgified Fabric API** via
**Sinytra Connector**. `side = "both"`.

**Beta:** needs in-game verification that Connector applies Vertigo's networking mixins
cleanly alongside Distant Horizons. If tall-world chunk holes persist or networking looks
wrong, treat Vertigo as the first suspect.
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.6-beta)
...
**Vertigo is `side = "both"`** (added in 0.6-beta). Dedicated servers and clients both install it.

**Too Fast is `side = "server"`** (added in 0.5.6). Dedicated-server packwiz syncs install it;
the default PrismLauncher pre-launch command (`--side client`) skips it. Singleplayer only
gets the rubber-band fix if the jar is actually in the instance.
