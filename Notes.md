...
## Overworld depth — height-patched Big Globe (as of 0.7.0, still in 0.7.8)
Current pack version is **0.7.8** (`pack.toml` matches). Overworld bounds are **floor −608**,
...
ore curves are unchanged from 0.6. **0.7.1–0.7.8 did not rebuild the jar** (datapack-only
...
- From **0.7.1 → 0.7.8** — **no** fresh world for height. Datapack-only WDA placement
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0–0.7.8):
...
## When Dungeons Arise (as of 0.7.8)
...
| `stattinkerer:large_dungeon` | 4 | 64 / 56 | massive dungeons (seated `keep_kayra` / `infested_temple` / `kisegi_sanctuary` at −15 + `beard_box`, flying `mechanical_nest`); 12-chunk exclusion vs villages |
...
Compat mismatches vs WDA 2.1.68 + this shallow world (author 0.7.0, depths retuned 0.7.2–0.7.8):
...
- Giant towers (`keep_kayra`, `infested_temple`, `kisegi_sanctuary`): all three currently
  **`start_height −15`**, `WORLD_SURFACE_WG`, **`beard_box`**, gated to `#bigglobe:land`.
  −15 seats the jigsaw start-anchor (which sits above each build's base) without crossing
  Big Globe's **`surface−16`** cave threshold (`test_cave`); at −16 or deeper the biome
  check flips underground and `#bigglobe:land` no longer matches → **zero spawns** (the
  **0.7.2** failure mode: infested −45 / kisegi −25). **0.7.7** reverted those two to
  surface **0** + `beard_box` so they generate again (`keep_kayra` stayed 0 /
  `beard_thin`). **0.7.8** seats all three at **−15** and puts `keep_kayra` on
  `beard_box` too so the foundation fills any exposed gap. A structure cannot be both
  *sunk below surface−16* and *land-gated* in BG. Do **not** restore infested/kisegi
  to −45/−25. Do **not** drop any of the three to −16 or deeper. **`mechanical_nest`**
  stays in `large_dungeon` but flies at Y 700–750. Those four remain gated to
  `#bigglobe:land`.
...
**0.7.2–0.7.8** did not change the 24/38 split (same 14 removed). They only retuned
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.7.8)
...
**0.6.8–0.7.8 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.7.8)
