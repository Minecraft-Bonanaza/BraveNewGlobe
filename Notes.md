...
## Overworld depth (as of 0.6)
Main uses **Big Globe default** overworld bounds (floor `-1024`, ceiling `+1024`, sea level 0).
The 0.6-beta shallow-overworld datapack and **Vertigo** are **not** in this pack. That work
is parked on branch `0.6-beta` to finish later (Connector networking-mixin + Distant Horizons
testing). Do not treat shallow bounds (`-464`…`+896`) as current.

If a player created a world during the brief window when 0.6-beta was on main, that save used
the compacted height. Starting a new world is the safe option after the revert.
...
**Creating Space is `side = "both"`** (added in 0.6). Dedicated servers and clients both
install it.
