...
Current pack version is **0.7.16** (`pack.toml` matches). Overworld bounds are **floor −608**,
...
ore curves are unchanged from 0.6. **0.7.1–0.7.16 did not rebuild the jar** (0.7.1–0.7.15
are datapack-only WDA hotfixes / placement retunes; **0.7.16** is Better Combat).
...
- From **0.7.1 → 0.7.16** — **no** fresh world for height. 0.7.1–0.7.15 are datapack-only
  WDA placement retunes; **0.7.16** adds Better Combat (no worldgen). Relaunch.
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0–0.7.16):
...
placement / heights / set membership (nest split in 0.7.12). **0.7.16** did not touch WDA.
...
**0.6.8–0.7.16 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.7.16)
...
Dungeons Arise, Better Combat, and the patched Big Globe jar are `side = "both"`.** Dedicated servers
