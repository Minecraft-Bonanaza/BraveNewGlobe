...
## Overworld depth — height-patched Big Globe (as of 0.7.0, still in 0.8.4)
Current pack version is **0.8.4** (`pack.toml` matches). **0.8.4** adds **LDLib2**
`2.2.37` and restores **owo-lib** `0.12.15.5-beta.1` (libraries; relaunch).
**PonderJS is not in the pack** (no 1.21.1 NeoForge build). **0.8.3** self-hosts
**Create: Villager Commerce** in `bundled-jars/`. **0.8.2** re-points **Create:
Linear Bearing** to Modrinth `1.3.5`. **0.8.1** / **0.8.0** / **0.7.19** add
**FTB Quests** (Create Core shipped). **0.7.17** / **0.7.18** swap the coin
economy to **Create: Numismatics**. Overworld bounds are **floor −608**,
...
ore curves are unchanged from 0.6. **0.7.1–0.8.4 did not rebuild the jar** (0.7.1–0.7.15
...
Linear Bearing's Modrinth re-point, and 0.8.4 libraries).
...
- From **0.7.1 → 0.8.4** — **no** fresh world for height. 0.7.1–0.7.15 are
  datapack-only WDA placement retunes; later versions add combat / economy /
  quests / libraries (no worldgen). Relaunch.
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0–0.8.4):
...
**0.7.2–0.8.4** did not change the 24/38 split (same 14 removed). 0.7.2–0.7.15 retuned
placement / heights / set membership (nest split in 0.7.12). **0.7.16–0.8.4** did not
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.8.4)
...
**0.6.8–0.8.4 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.8.4)
...
FTB Quests / FTB Library / FTB Teams, LDLib2, owo-lib, and
...
**0.7.16**, in the same commit as **`MODLIST.md`**. Current `pack.toml` is **0.8.4**.
...
**`MODLIST.md`** is a human-readable grouped list of all **141** mods (as of 0.8.4;
129 when first added, 135 as of 0.7.18, 136 after Villager Commerce).
...
`pack.toml` is **0.8.4**. `side = "both"`. No worldgen. **No fresh world.**
...
[`MODLIST.md`](MODLIST.md) (count is **141** as of 0.8.4).
...
**PonderJS is not in the pack.** There is no 1.21.1 NeoForge build on CurseForge
or Modrinth (upstream stalled ~1.20). Pending an upstream release or a
self-hosted jar. Do not write quests that assume in-world Ponder scenes.
