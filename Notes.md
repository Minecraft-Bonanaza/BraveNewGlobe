...
## Current pack state (1.1.21)

Living rules. CHANGELOG stays historical.

- **Version / count:** `pack.toml` **1.1.21**, **221** mods. Quest book **18 / 251**
...
- **1.1.17:** **Healight** required on both sides (was client optional). It defines living-entity synched data; client-only install desyncs field 16 on dedicated servers.
...
- **1.1.12:** **ParCool 3.4.3.3** + **ParCool+ 1.2.1** (not 4.x). **Counter 0.5** day HUD (disable coords/FPS/time overlays). **Beautified Chat Server 3.2** + **Collective 8.40** (no Client jar). **Simplest Paxels 1.0.6**.
- **1.1.10:** **Chunky** pulled. **Distant Horizons** rolled back to **3.2.0-b** (was 3.3.1). Auto-updater off.
...
- **1.1.8:** shipped the CANDIDATES pending set (13 mods). Dragons Plus **1.11.9**,
  ...
  Integrated Farming, and Petrolpark load. Way Better Title Bar is client optional, default off.
- **1.1.7:** KubeJS sprinkler hydrator restored (1.1.4 player tick) alongside the Java jar.
...
- **1.1.3:** sprinklers wet farmland on farmland randomTick (after RF), LOS air/crops in radius.
...
- **1.1.1:** **MDT splint** lasts **2 minutes** (was infinite / ~80 min hidden
  ...
  water sprinklers** hydrate Realistic Farmland (`minecraft:farmland` moisture 7) via
  KubeJS `sprinkler_hydrate_farmland.js` (`/reload`).
