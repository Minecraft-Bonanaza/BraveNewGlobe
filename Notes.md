...
## Overworld depth — height-patched Big Globe (as of 0.7.0, still in 0.7.16)
Current pack version is **0.7.16** (`pack.toml` matches), plus an **unversioned** add after
0.7.16 (**Create: Linear Bearing**, **VS / Sable Hose Connectors**, and **MODLIST.md**).
...
ore curves are unchanged from 0.6. **0.7.1–0.7.16 and the unversioned add after did not
rebuild the jar** (0.7.1–0.7.15 are datapack-only WDA hotfixes / placement retunes;
**0.7.16** is Better Combat; the unversioned add is two Create/Sable mods).
...
- From **0.7.1 → 0.7.16** (and the unversioned add after) — **no** fresh world for height.
  0.7.1–0.7.15 are datapack-only WDA placement retunes; **0.7.16** adds Better Combat;
  the unversioned add is two Create/Sable mods (no worldgen). Relaunch.
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0–0.7.16 and
the unversioned add after):
...
## When Dungeons Arise (as of 0.7.15, still in 0.7.16)
...
**0.7.2–0.7.16** did not change the 24/38 split (same 14 removed). 0.7.2–0.7.15 retuned
placement / heights / set membership (nest split in 0.7.12). **0.7.16** and the unversioned
add after did not touch WDA.
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.7.16)
...
**0.6.8–0.7.16 and the unversioned add after did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.7.16 / unversioned after)
...
Dungeons Arise, Better Combat, Create: Linear Bearing, VS / Sable Hose Connectors, and the
patched Big Globe jar are `side = "both"`.** Dedicated servers
...
`pack.toml` is still **0.7.16**. These two CurseForge mods landed in the same author commit
as **`MODLIST.md`** (`00a0f01`). Both are `side = "both"`. No worldgen. **No fresh world.**
...
**`MODLIST.md`** is a human-readable grouped list of all **129** mods (including these two).
Dependency nestings there are best-effort. packwiz `pack/mods/*.pw.toml` remains the source
of truth. Update `MODLIST.md` when adding or removing mods.
