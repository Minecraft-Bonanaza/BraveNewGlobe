...
## Overworld depth — height-patched Big Globe (as of 0.7.0, still in 0.7.15)
Current pack version is **0.7.15** (`pack.toml` matches). Overworld bounds are **floor −608**,
...
ore curves are unchanged from 0.6. **0.7.1–0.7.15 did not rebuild the jar** (datapack-only
...
- From **0.7.1 → 0.7.15** — **no** fresh world for height. Datapack-only WDA placement
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0–0.7.15):
...
## When Dungeons Arise (as of 0.7.15)
...
**0.7.2–0.7.15** did not change the 24/38 split (same 14 removed). They retuned
placement / heights / set membership (nest split in 0.7.12). Datapack-only; relaunch.
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.7.15)
...
**0.6.8–0.7.15 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.7.15)
