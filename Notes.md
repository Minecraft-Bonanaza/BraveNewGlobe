...
## Overworld depth — height-patched Big Globe (as of 0.7.0, still in 0.7.1)
Current pack version is **0.7.1** (`pack.toml` matches). Overworld bounds are **floor −608**,
...
ore curves are unchanged from 0.6. **0.7.1 did not rebuild the jar** (datapack-only hotfix).
...
Current underground layout (from the 0.5.9 / 0.6 jar; floor unchanged in 0.7.0 / 0.7.1):
...
## When Dungeons Arise (as of 0.7.0 / 0.7.1)
...
missing pool. Do **not** restore that def. The 24/38 set split is unchanged (this def was
never in a set). Datapack-only — relaunch; no fresh world for the hotfix itself.
...
## Glacier ice (`bigglobe_less_glacier`, as of 0.6.7, still in 0.7.1)
...
**0.6.8–0.7.1 did not touch glaciers.**
...
## Client-only / server-only mods (packwiz `side`, as of 0.5.2–0.7.1)
