...
### Documentation
- **Notes** — living glacier rules now describe v2 `1.4 × (glacier_crack_threshold − 0.25)`
  sheets (Voronoi variation 40), not 0.6.1's `glacier_field` or the 0.5 `0.4×` uniform-floe
  grid. Overworld depth, C2ME + Vertigo, CTOV villages, and packwiz `side` flags are
  unchanged from 0.6.
- **README / client install** — pack version is **0.6.2**. The glacier change is datapack-only
  (no fresh world for players already on 0.6); explore new cold-ocean chunks to see ice.
  0.6.1 chunks that generated with no ice stay empty until regenerated. Pre-0.6 still needs
  a new world for the −608 floor.
- **Maintainer** — do not re-add `glacier_field.json`. Knobs are the `0.25` cutoff and `1.4`
  multiplier. Historical 0.5.2–0.5.5 / Too Fast / Creating Space / 0.6-beta entries filled
  below.
