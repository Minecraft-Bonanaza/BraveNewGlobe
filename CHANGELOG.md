...
Fix packwiz file hashes. Git was rewriting line endings (CRLF ↔ LF) on checkout, which changed
file bytes under `pack/` so `index.toml` hashes no longer matched. Players then failed the
packwiz installer hash check on launch. No mods, configs, or datapack *content* changed.
...
### Documentation
- **Maintainer + Notes** — line-ending / `.gitattributes` gotcha; `packwiz refresh` is still
  required after real file changes.
- **Client install** — hash-mismatch troubleshooting updated (this was the usual cause).
