...
This file tracks mod additions/removals, mod version updates, config/pack changes,
and notable documentation.

## Unversioned (after 0.7.18) — Create: Villager Commerce — 2026-08-29

`pack.toml` version string is still **0.7.18**. Author commit `5dce09f` ("Added Villager Commerce")
did not bump the pack version.

### Added (mods)
- **Create: Villager Commerce** `1.0.1` (CurseForge 1557618 / file 8183116,
  `pack/mods/create-villager-commerce.pw.toml`, `side = both`) — Merchant Stalls linked to
  Create stock networks so **villagers automatically buy from player shops**. Markets are
  managed with Market Ledgers (daily purchase limits, preset trades, villager registration).
  Purchases are **simulated** (sold item leaves stock, payment is inserted; villagers do not
  keep the goods). Optional **Create: Numismatics** currency is already in the pack from
  [0.7.17] — preset trades pay in coins; without Numismatics the mod falls back to iron
  ingots. Required dep already present: **Create**. This is the player-shop / villager-customer
  direction; **Villager Currency** (0.7.18) remains the opposite (players buy from villagers).
- **`MODLIST.md`** updated (**136** mods). SIG home is Commerce & Markets in
  [`SPECTRUM.md`](SPECTRUM.md). **No fresh world.** Relaunch so packwiz adds the jar.

## Unversioned (after 0.7.18) — SPECTRUM.md — 2026-08-27

`pack.toml` version string is still **0.7.18**. Author commit `41ec107` ("Spectrum.md")
did not bump the pack version and did not add mods.

### Added (docs)
- **`SPECTRUM.md`** — Special Interest Group (SIG) framework for designing **FTB Quests**
  around player specialization. Fifteen SIGs (Metalworking through Civil Works), each with a
  baseline → specialized curve, core mods, quest beats, and exclusions. Also: non-SIG layers
  (world / fauna / QoL / libraries), a copy/paste quest writing template, a cross-SIG map,
  and a suggested FTB Quests book structure. README links it.
- **FTB Quests is not in the pack.** This is a design document for quest authors, not a
  shipped questbook. Currency stays **earned** (villagers, bounties, deliveries, player
  trade) — do not quest player-minted coins or remote / mailbox checkout.
- No worldgen / height / mod change. **No fresh world.**
