...
experiment (**Vertigo** + **`bigglobe_shallow_overworld`**) is **parked on branch
`0.6-beta`** and removed from main so it can finish later. Overworld returns to Big Globe
default depth. Creating Space stays. The author did not bump `pack.toml`; the version
string is still `0.6-beta`.
...
- `pack.toml` version left at **0.6-beta**.
- `pack/index.toml` + `pack/pack.toml` re-indexed.

### Documentation
- **Notes** — Creating Space living note (own planet dims, no BG datapack). 0.6-beta
  performance work is parked on branch `0.6-beta`; overworld is BG default depth again.
- **README / client install** — Creating Space is in the pack; current main does not
  require a fresh world. Worlds created during the brief shallow-overworld window should
  start fresh. packwiz will delete Vertigo on the next launch.
- **Maintainer** — Creating Space is Modrinth-only, `side = "both"`. Do not re-add
  Vertigo / shallow-overworld to main until the parked branch is finished.

## [0.6-beta] — 2026-08-23

Performance beta (now **parked on branch `0.6-beta`**): shrink Big Globe's oversized
overworld and add vertical-section syncing, targeting on-disk world size and slow chunk
streaming ahead of the player (gaps between loaded chunks and Distant Horizons).

Removed from main in the preceding [0.6] commit; kept here as history.
...
### Documentation
- **Maintainer + Notes** — packwiz `side = "server"` documented; Too Fast is the
  current server-only mod. Client-only list is unchanged (JEI stays `both`;
  MapStitch stays out).
- **README / client install** — dedicated-server notes mention Too Fast installs
  on the server; default client sync skips `side = "server"` mods.
...
### Documentation
- **Maintainer + Notes** — MapStitch is gone; only **JEI** must stay `side = "both"`
  among the mods that 0.5.2 briefly marked client-only.
- **README / client install** — dedicated-server notes no longer mention MapStitch.
