...
### Documentation
- **Notes** — living glacier rules now describe `min(1.4 × (crack_threshold − 0.60), 0.75)`:
  ice only over the **deep/cold core** (0.60 cutoff), broken into large floes (0.75 cap) —
  not v3's continent-sized sheet, not v1 `glacier_field`, not the 0.5 `0.4×` grid.
  Overworld depth, C2ME + Vertigo, CTOV villages, and packwiz `side` flags are
  unchanged from 0.6.
- **README / client install** — pack version is **0.6.4**. The glacier change is
  datapack-only (no fresh world for players already on 0.6); explore new cold-ocean
  chunks. 0.6.1 no-ice chunks and 0.6.2/0.6.3 larger sheets stay until regenerated.
  Pre-0.6 still needs a new world for the −608 floor.
- **Maintainer** — do not re-add `glacier_field.json`. Size dial is the `0.60` cutoff
  (higher → smaller footprint); `0.75` cap keeps the main mass as broken floes.
