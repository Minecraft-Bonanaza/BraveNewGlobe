...
Two mods added. **Corpse** (loot-your-body on death) and **Create Aeronautics: Throwable Rope
Connector**. Drive-By-Wire With Sable was briefly indexed then dropped in the same version (optional
Aeroworks dep; not needed). `pack.toml` version field caught up (was stuck at `0.6.7` through
0.6.8–0.6.10).
...
- **Corpse** (henkelmax) `neoforge-1.21.1-1.1.13` (CurseForge 316582 / file 7018307) — on death, spawns
  ...
  list + optional death waypoint. Standalone (Jade/OpenHUD deps optional; Jade already in pack;
  OpenHUD is not). `side = "both"`.
- **Create Aeronautics: Throwable Rope Connector** `0.4.3` (CurseForge 1529882 / file 8618590) — a
  Create Aeronautics add-on. Required deps already satisfied by the pack (Create 6.0.10,
  `aeronautics_bundled` 1.3.1). `side = "both"`.

### Removed (same version)
- **Drive-By-Wire With Sable** `0.3.0` (CurseForge 1520378 / file 8247104) — packwiz refresh in the
  Corpse commit had indexed this uncommitted `.pw.toml`; it was committed then immediately removed.
  `drivebywire` is only an optional Aeroworks dep. **Sable stays** (required by Aeroworks).

### Config / pack
- Added `pack/mods/corpse.pw.toml` + `pack/mods/create-aeronautics-throwable-rope-connector.pw.toml`.
- `pack.toml` version field: `0.6.7` → `0.6.10` (immediately preceding metadata-only commit, catching
  up 0.6.8–0.6.10) → **`0.6.11`**. `pack/index.toml` + `pack/pack.toml` re-indexed.

### Documentation
- **Notes** — Corpse + Throwable Rope Connector are in; Drive-By-Wire is **not**. Sable stays.
  `pack.toml` version string is now **0.6.11** (no longer lagged at 0.6.7). Overworld height, glaciers,
  C2ME-out / Vertigo, In Control Y ≤ −50 clamp, CTOV villages, Creating Space, Too Fast, and the rest
  of the packwiz `side` flags are unchanged from 0.6.10.
- **README / client install** — pack version is **0.6.11**. No fresh world if already on 0.6.
  packwiz-installer adds the two new jars on next launch.
- **Maintainer** — keep Corpse and Throwable Rope Connector `side = "both"`. Do not re-add
  `drive-by-wire-with-sable.pw.toml`. Do not drop Sable (Aeroworks requires it). Historical
  0.5.2–0.5.5 / Too Fast / Creating Space / 0.6-beta entries filled below.
...
### Documentation
- **Notes** — **C2ME is out** (0.6.10); **Vertigo stays**. Large Sea Myths (kraken / leviathan /
  sea_eater) are In Control–clamped to **Y ≤ −50** and stattinkerer weight **2** (0.6.8–0.6.9).
  Overworld height, glaciers, CTOV villages, Creating Space, Too Fast, and the rest of the
  packwiz `side` flags are unchanged from 0.6.7. At this commit the `pack.toml` version field
  was still `0.6.7` (hash re-indexed; version field not bumped in 0.6.8–0.6.10); the next
  metadata-only commit bumped it to `0.6.10`, then 0.6.11 set it to `0.6.11`.
- **README / client install** — pack version at this commit is **0.6.10**. No fresh world for
  players already on 0.6 (0.6.8–0.6.10 are a spawn-rule mod + datapack + a leaf-mod removal).
  packwiz-installer deletes the C2ME jar on next launch. Pre-0.6 still needs a new world for
  the −608 floor.
- **Maintainer** — do not re-add `c2me.pw.toml` unless the Vertigo mixin overlap is resolved
  (or Vertigo is also dropped). Keep Vertigo and In Control `side = "both"`. Spawn clamp lives
  in `pack/config/incontrol/spawn.json`; weights in `stattinkerer_bigglobe_compat`.
