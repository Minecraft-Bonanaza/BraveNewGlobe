...
### Documentation
- **Notes** — living glacier rules now describe `glacier_field` sheets that thin into satellite
  floes (Voronoi variation 40), not the 0.5 `0.4×` uniform-floe grid. Overworld depth, C2ME +
  Vertigo, CTOV villages, and packwiz `side` flags are unchanged from 0.6.
- **README / client install** — pack version is **0.6.1**. The glacier change is datapack-only
  (no fresh world for players already on 0.6); explore new cold-ocean chunks to see it. Pre-0.6
  still needs a new world for the −608 floor.
- **Maintainer** — `bigglobe_less_glacier.zip` is experimental / unverified Big Globe script;
  watch the log on first load. Historical 0.5.2–0.5.5 / Too Fast / Creating Space / 0.6-beta
