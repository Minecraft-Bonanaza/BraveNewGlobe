...
### Documentation
- **Notes** — living glacier rules now describe **fewer + smaller solid sheets**: temp bar
  `unmixLinear(-0.35, -0.6)` (rarer oceans) and cutoff **C = 0.4** on *both* the ice feature
  and the biome decision tree so footprints match (Aquamirae shrinks too). Not 0.6.5/0.6.2
  continent-sized v2 sheets, not 0.6.4 broken-floe deep-core ice, not 0.6.1 `glacier_field`,
  not the 0.5 `0.4×` grid. Overworld depth, C2ME + Vertigo, CTOV villages, and packwiz `side`
  flags are unchanged from 0.6.
- **README / client install** — pack version is **0.6.6**. The glacier change is datapack-only
  (no fresh world for players already on 0.6); explore new cold-ocean chunks. Older glacier
  chunks (0.6.1 empty, 0.6.2/0.6.5 continent, 0.6.3 whole-ocean floes, 0.6.4 core-only floes)
  stay until regenerated. Pre-0.6 still needs a new world for the −608 floor.
- **Maintainer** — do not re-add `glacier_field.json`. Do not restore the `0.75` cap or the
  0.6.4 `0.60`-only feature cutoff as current. Keep ice + biome on the same C = 0.4 formula.
  Frequency dial is the temp bar; size dial is C (higher → smaller). If the biome does not
  shrink in-game, move `shallow_ocean_test_glacier.json` into the patched jar. Historical
