...
### Documentation
- **Notes** — living glacier rules now describe fewer + smaller solid sheets with the **0.6.7**
  temperature bar `unmixLinear(-0.4, -0.65)` (frequency dial: a few fewer glacier oceans).
  Size stays cutoff **C = 0.4** on ice *and* biome so footprints match (Aquamirae shrinks too).
  Not 0.6.6's `-0.35/-0.6` bar, not 0.6.5/0.6.2 continent-sized v2 sheets, not 0.6.4
  broken-floe deep-core ice, not 0.6.1 `glacier_field`, not the 0.5 `0.4×` grid. Overworld
  depth, C2ME + Vertigo, CTOV villages, and packwiz `side` flags are unchanged from 0.6.
- **README / client install** — pack version is **0.6.7**. The glacier change is datapack-only
  (no fresh world for players already on 0.6); explore new cold-ocean chunks. Older glacier
  chunks stay until regenerated. Pre-0.6 still needs a new world for the −608 floor.
- **Maintainer** — frequency dial is now `-0.4 / -0.65`. Do not re-add `glacier_field.json`.
  Do not restore the `0.75` cap or the 0.6.4 `0.60`-only feature cutoff as current. Keep ice
  + biome on the same C = 0.4 formula. Size dial is still C (higher → smaller). If the biome
  does not shrink in-game, move `shallow_ocean_test_glacier.json` into the patched jar.
  Historical 0.5.2–0.5.5 / Too Fast / Creating Space / 0.6-beta entries filled below.
