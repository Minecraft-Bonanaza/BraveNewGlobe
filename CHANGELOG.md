...
- `pack.toml` version stays **0.9.15** (this commit rolled the 0.9.16 version string back).
  The **Create: Warnautics 1.0.8** jar from [0.9.16] stays. Pack still **158** mods.
  Living menu: `LOOT.md`.
...
- The later [0.9.15] LootJS hotfix rolled `pack.toml` back to **0.9.15**; this jar bump remains.
...
- Follow-ups in the headings above: Warnautics **1.0.8** so `cruise_missile` resolves ([0.9.16]);
  `aquamirae:oxygen_tank` removed so the script actually loads (hotfix [0.9.15]).
