...
- **Pack version:** `0.7.16` (`pack.toml`), plus an **unversioned** add after 0.7.16
  (Create: Linear Bearing, VS / Sable Hose Connectors, and [MODLIST.md](MODLIST.md))
...
## Worlds — unversioned Linear Bearing / Hose Connectors vs 0.7.16 Better Combat vs 0.7.15 placement vs 0.7.1 hotfix vs 0.7.0 ceiling vs 0.6 height
...
across Sable ships), plus a human-readable **[MODLIST.md](MODLIST.md)**. `pack.toml` is
still **0.7.16**. They do **not** change world height or structure placement. Relaunch
so packwiz downloads the jars. **No new world.**
...
**0.6.1–0.6.11 did not change height.** 0.7.0 does (ceiling only). 0.7.1–0.7.16 and the
unversioned add after do not.
...
- **Unversioned (after 0.7.16)** adds **Create: Linear Bearing** `1.2.6` (Sable physics
  ...
  Sable ships), plus **[MODLIST.md](MODLIST.md)** (129 mods, grouped). `pack.toml` is
  still 0.7.16. Relaunch; no new world.
...
- **Dedicated server** — client-only mods (Iris, Sodium, ImmediatelyFast, Mod Menu,
  ...
  **Create: Linear Bearing**, **VS / Sable Hose Connectors**, and the patched **Big Globe**
...
- **DH still cuts off at +896 after 0.7.0** — the ceiling moved in the Big Globe jar
  in 0.7.0 (0.7.1 onward, including the unversioned add after 0.7.16, did not change
  height). Clear the Distant Horizons LOD cache and relaunch (or start a new world).
