...
## Worlds — 0.6 height vs 0.6.1 glaciers
...
**0.6.1** only changes ocean glacier ice (datapack `bigglobe_less_glacier`). If you are already on
a 0.6 world, **keep it**. Already-generated cold ocean stays on the old floe grid; explore
**new** ocean (or delete those chunks) to see the sparse sheets and thinning satellite floes.
On first load after the update, if glaciers error in the log, ping the maintainer — this
datapack is experimental.
