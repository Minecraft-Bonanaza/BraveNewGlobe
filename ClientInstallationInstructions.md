...
## Worlds — 0.6 height vs 0.6.4 glaciers
...
**0.6.1–0.6.4** only change ocean glacier ice (datapack `bigglobe_less_glacier`). If you are
already on a 0.6 world, **keep it**.

- **0.6.1** generated **no ice** (the new `glacier_field` never cleared its cutoff).
- **0.6.2** put ice back as a dense-core sheet that thinned toward the shallows — but the
  core merged into a **continent-sized** mass.
- **0.6.3** capped fill at `0.75` so that mass broke into large floes, still covering the
  whole cold ocean.
- **0.6.4** raises the cutoff to `0.60` so ice is **broken floes over the deep/cold core
  only**.

Already-generated cold ocean stays as it was; explore **new** ocean (or delete those chunks)
to see the 0.6.4 ice.
