...
## Worlds — 0.7.1 hotfix vs 0.7.0 ceiling vs 0.6 height vs later glacier / spawn updates
**0.7.1** is a **datapack-only hotfix**. It does **not** change world height. Relaunch so
packwiz pulls the updated `bigglobe_whendungeonsarise.zip`. **"Create New World" works
again** — 0.7.0 crashed at registry load because the BG×WDA compat pack still defined
`small_prairie_house`, whose template pool is gone in When Dungeons Arise 2.1.68.
...
**0.6.1–0.6.11 did not change height.** 0.7.0 does (ceiling only). 0.7.1 does not.
...
- **DH still cuts off at +896 after 0.7.0 / 0.7.1** — the ceiling moved in the Big Globe jar
  in 0.7.0 (0.7.1 did not change height). Clear the Distant Horizons LOD cache and relaunch
  (or start a new world). The floor is still −608.
