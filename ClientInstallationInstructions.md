...
## Worlds — 0.7.8 placement vs 0.7.1 hotfix vs 0.7.0 ceiling vs 0.6 height
**0.7.2–0.7.8** are **datapack-only** When Dungeons Arise placement retunes. They do
...
**0.6.1–0.6.11 did not change height.** 0.7.0 does (ceiling only). 0.7.1–0.7.8 do not.
...
- **0.7.4** retunes underground depths: **scorched_mines −150**, **plague_asylum −300**,
  ...
  **0** + **`beard_box`** so they generate again (0.7.2's −45/−25 sank the biome
  check underground vs `#bigglobe:land` → zero spawns). **0.7.8** seats
  `infested_temple` / `kisegi_sanctuary` / `keep_kayra` at surface **−15** with
  **`beard_box`** (just above BG's `surface−16` cave threshold). Relaunch; no new
  world.
...
regenerated. Already-generated 0.7.0–0.7.7 WDA structures keep their old positions until
those chunks are regenerated. Worlds created on **0.7.2–0.7.6** likely never spawned
`infested_temple` / `kisegi_sanctuary` (biome mismatch); after 0.7.7+ explore **new**
land (or regenerate those chunks) to get them.
...
- **DH still cuts off at +896 after 0.7.0–0.7.8** — the ceiling moved in the Big Globe jar
  in 0.7.0 (0.7.1–0.7.8 did not change height). Clear the Distant Horizons LOD cache and
