...
## Worlds — 0.6 height vs later glacier / spawn / C2ME updates
...
**0.6.1–0.6.10 do not change height.** If you are already on a 0.6 world, **keep it**.
...
- **Dedicated server** — client-only mods (Iris, Sodium, ImmediatelyFast, Mod Menu,
  ...
  on a dedicated server. **JEI**, **Creating Space**, **Vertigo**, **In Control!**, and the patched
  **Big Globe** jar are `side = "both"` and do install on the server. **Too Fast** is
  ...
  pre-launch command above). **C2ME is not in the pack.** MapStitch is not in the pack.
