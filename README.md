...
- **Current pack version:** `0.8.7` (`pack/pack.toml` matches). **0.8.7**
  reworks **Create Core** to **12 purpose-driven quests** (use a machine to
  produce an output or do a task; capstone = a passive Andesite Alloy line)
  with bootstrap-kit rewards. Same **145** mods. **0.8.6**
  re-refreshes the stale `pack.toml` `[index]` hash left by 0.8.5 (packwiz
  **"index hash file invalid"**). No mod changes. **0.8.5** adds
  **Ponder for KubeJS** (`ponderjs` 1.21.1-2.4.0, slug **`ponder`**) plus
  **KubeJS** `2101.7.2`, **Rhino** `2101.2.8`, and **Better Advanced Tooltips**
  `2101.1.0` — the missing required dep chain for **Delivery Required** (the pack
  crashed on load without them). **0.8.4** adds **LDLib2** `2.2.37` and restores
  **owo-lib** `0.12.15.5-beta.1`. **0.8.3** self-hosts **Create: Villager Commerce**
  in `bundled-jars/` (CF distribution disabled). **0.8.2** re-points **Create:
  Linear Bearing** CurseForge → Modrinth (`1.2.6` → `1.3.5`). **0.8.1** makes
  FTB Quests IDs deterministic. **0.8.0** ships the **Create Core** guidance
  chapter + **[QUESTS.md](QUESTS.md)**. **0.7.19** adds **FTB Quests** + FTB
  Library + FTB Teams. **No fresh world** from 0.7.1 onward (including
  0.7.16–0.8.7). **Fresh world required** if you are
...
- **Survival & QoL** — seasons, thirst/diseases, farming and food, **ChoiceTheorem's Overhauled
  ...
  **FTB Quests** is in (guidance book; Create Core is **12 purpose-driven
  quests** as of 0.8.7). **owo-lib** is back
  (restored 0.8.4; it is not Overhaul-only). **LDLib2** is in. **PonderJS is in**
...
- **[Mod List](MODLIST.md)** — human-readable list of every mod in the pack (145), grouped by purpose.
...
- **[QUESTS.md](QUESTS.md)** — locked 15-line guidance roadmap. **Create Core** is
  **12 purpose-driven quests** (0.8.7); other lines pending. **PonderJS is in** (Delivery Required dep).
- **[Notes](Notes.md)** — current overworld-depth / village / glacier / WDA rules, Vertigo (C2ME
  ...
  FTB Quests / Create Core (0.8.7 purpose-driven rework), LDLib2 / owo-lib, PonderJS / KubeJS chain, Better Combat,
  Creating Space, SPECTRUM / SIG quest-design notes, packwiz `side` flags,
  line-ending/hash gotcha (and the 0.8.6 stale-`pack.toml`-hash fix), plus misc config notes.
...
4. **Start a new world** if you are upgrading from 0.6.x (ceiling raised to **+1024**; or
   ...
   **0.7.1–0.8.7** do not change height — relaunch so the 0.8.7 Create Core
   rework lands, then the 0.8.6 hash fix, the PonderJS / KubeJS
   chain, LDLib2, owo-lib, the self-hosted Villager Commerce jar, Linear Bearing
   1.3.5, FTB Quests, Numismatics / Delivery Required, Hose
   Connectors, Better Combat, and the latest WDA compat datapack install.
