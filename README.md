...
- **Current pack version:** `0.6.6` (`pack/pack.toml`). **Fresh world required** if you are
  upgrading from any version before **0.6** (overworld height changed). 0.6.1–0.6.6 are
...
- **Big Globe world generation** — height-patched 5.3.2 jar
  ...
  **+896** (DH-safe; the height lives inside BG's own jar). Ocean glaciers are **fewer and
  smaller solid sheets** (`bigglobe_less_glacier`: colder temp bar `unmixLinear(-0.35, -0.6)`
  plus ice *and* biome gated to `1.4 × (crack_threshold − 0.4)`). The glacier biome matches
  the ice, so Aquamirae's Sea of Shivers shrinks with them. 0.6.5 restored the solid-sheet
  look (no `0.75` cap); 0.6.4's broken-floe core and 0.6.1's `glacier_field` are not current.
...
4. **Start a new world** if you are upgrading from any pack version before 0.6 (height changed).
   ...
   already on 0.6, keep that world — then explore **new** cold ocean to see 0.6.6 glaciers.
