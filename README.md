...
- **Current pack version:** `0.7.16` (`pack/pack.toml` matches), plus an **unversioned** add
  after 0.7.16: **Create: Linear Bearing**, **VS / Sable Hose Connectors**, and
  **[MODLIST.md](MODLIST.md)** (relaunch; no worldgen change). **0.7.16** adds **Better Combat**
  (relaunch; no worldgen change). **0.7.2–0.7.15** are datapack-only WDA placement retunes
  (relaunch; no height change). **Fresh world required**
...
- **Create ecosystem** — Create plus a large stack of add-ons (aeronautics, cannons, logistics,
  diesel generators, trains, and more). **Create: Linear Bearing** adds Sable physics sliders
  (cranes / elevators). **VS / Sable Hose Connectors** transfer fluids, items, rotation, and
  energy across Sable physics ships. **Creating Space** adds Create-based rockets and travel
...
- **Survival & QoL** — seasons, thirst/diseases, farming and food, **ChoiceTheorem's Overhauled
  ...
  **Create Aeronautics: Throwable Rope Connector**, **Create: Linear Bearing**, and
  **VS / Sable Hose Connectors** are in; Drive-By-Wire With Sable is not (optional Aeroworks
  dep, dropped in 0.6.11). Sable stays.
...
- **[Mod List](MODLIST.md)** — human-readable list of every mod in the pack (129), grouped by purpose.
...
- **[Notes](Notes.md)** — current overworld-depth / village / glacier / WDA rules, Vertigo (C2ME
  ...
  Hose Connectors, Better Combat, Creating Space, packwiz `side` flags, line-ending/hash
...
4. **Start a new world** if you are upgrading from 0.6.x (ceiling raised to **+1024**; or
   ...
   **0.7.1–0.7.16 and the unversioned add after** do not change height — relaunch so
   Linear Bearing, Hose Connectors, Better Combat, and the latest WDA compat datapack
   install.
