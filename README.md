...
- **Current pack version:** `0.7.8` (`pack/pack.toml` matches). **0.7.2–0.7.8** are
...
- **Adventure structures** — **When Dungeons Arise** `2.1.68` with a patched Big Globe compat
  ...
  **0.7.8** seats those two plus `keep_kayra` at surface **−15** with **`beard_box`**
  (stays above BG's `surface−16` cave threshold so `#land` still matches).
...
4. **Start a new world** if you are upgrading from 0.6.x (ceiling raised to **+1024**; or
   ...
   **0.7.1–0.7.8** do not change height — relaunch so the latest WDA compat datapack installs.
