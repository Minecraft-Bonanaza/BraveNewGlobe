...
- **Current pack version:** `0.6.1` (`pack/pack.toml`). **Fresh world required** if you are
  upgrading from any version before **0.6** (overworld height changed). 0.6.1 is a glacier
  datapack tweak and does not need a new world if you are already on 0.6.
...
- **Big Globe world generation** — height-patched 5.3.2 jar
  ...
  **+896** (DH-safe; the height lives inside BG's own jar). Ocean glacier *ice* is experimental
  v1: sparse **sheets that thin into satellite floes** and fade to open water (`glacier_field`
  in `bigglobe_less_glacier`); the glacier biome itself is unchanged. Hand-authored
...
4. **Start a new world** if you are upgrading from any pack version before 0.6 (height changed).
   ...
   already on 0.6, keep that world — then explore **new** cold ocean to see 0.6.1 glaciers.
