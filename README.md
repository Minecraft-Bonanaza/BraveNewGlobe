...
- **Current pack version:** `0.7.9` (`pack/pack.toml` matches). **0.7.2–0.7.9** are
...
- **Adventure structures** — **When Dungeons Arise** `2.1.68` with a patched Big Globe compat
  ...
  **0.7.9** puts those two plus `keep_kayra` back on surface **0** with **`beard_box`**
  (proven 0.7.0/0.7.7 anchor; full 16-block margin above BG's `surface−16` cave
  threshold so `#land` still matches). Do not re-apply 0.7.8's −15 seating.
...
4. **Start a new world** if you are upgrading from 0.6.x (ceiling raised to **+1024**; or
   ...
   **0.7.1–0.7.9** do not change height — relaunch so the latest WDA compat datapack installs.
