...
- **Current pack version:** `0.7.18` (`pack/pack.toml` matches). An **unversioned** add
  after 0.7.18 is **Create: Villager Commerce** `1.0.1` (villagers buy from player
  Merchant Stalls; relaunch; no worldgen). Another **unversioned** add after 0.7.18
  is **[SPECTRUM.md](SPECTRUM.md)** (SIG / FTB Quests design doc; **FTB Quests
  is not in the pack**). **0.7.18** / **0.7.17** swap the coin economy to **Create:
  Numismatics** (plus Delivery Required, villager-coin trades, shops / stock market —
  relaunch; no worldgen change). An **unversioned** add after 0.7.16 still includes
  **Create: Linear Bearing**, **VS / Sable Hose Connectors**, and
...
- **Create ecosystem** — Create plus a large stack of add-ons (aeronautics, cannons, logistics,
  ...
  villagers buy from player Merchant Stalls on a Create stock network. **Create: Linear Bearing** adds Sable
...
- **[Mod List](MODLIST.md)** — human-readable list of every mod in the pack (136), grouped by purpose.
...
- **[Maintainer Instructions](MaintainerInstructions.md)** — how the pack is built and how to add/update
  mods with packwiz.
- **[SPECTRUM.md](SPECTRUM.md)** — Special Interest Group framework for a future **FTB Quests**
  book (baseline → specialized careers). **FTB Quests is not in the pack**; this is a design
  document, not a shipped questbook.
- **[Notes](Notes.md)** — current overworld-depth / village / glacier / WDA rules, Vertigo (C2ME
  ...
  Better Combat, Creating Space, SPECTRUM / SIG quest-design notes, packwiz `side` flags,
  line-ending/hash gotcha,
...
4. **Start a new world** if you are upgrading from 0.6.x (ceiling raised to **+1024**; or
   ...
   **0.7.1–0.7.18** (including the unversioned Linear Bearing / Hose Connectors add after
   0.7.16 and Villager Commerce after 0.7.18) do not change height — relaunch so
   Villager Commerce, Numismatics / Delivery Required, Linear Bearing, Hose
