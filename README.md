...
- **Current pack version:** `0.6.4` (`pack/pack.toml`). **Fresh world required** if you are
  upgrading from any version before **0.6** (overworld height changed). 0.6.1–0.6.4 are
...
- **Big Globe world generation** — height-patched 5.3.2 jar
  ...
  **+896** (DH-safe; the height lives inside BG's own jar). Ocean glacier *ice* is
  **broken floes over the deep/cold core** (`bigglobe_less_glacier`:
  `min(1.4 × (crack_threshold − 0.60), 0.75)`). 0.6.1's `glacier_field` produced no ice;
  0.6.2/0.6.3 put ice back but covered the whole cold ocean. The glacier biome itself is
  unchanged. Hand-authored compatibility datapacks cover spawns, structures, and cross-mod
  integration (shipped via Paxi). The old `bigglobe_shallow_overworld` *datapack* is **not**
  in the pack (that approach caused the 0.6-beta Distant Horizons offset).
...
4. **Start a new world** if you are upgrading from any pack version before 0.6 (height changed).
   ...
   already on 0.6, keep that world — then explore **new** cold ocean to see 0.6.4 glaciers.
