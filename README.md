...
- **Current pack version:** `0.7.1` (`pack/pack.toml` matches). **0.7.1** is a datapack hotfix
  (relaunch; "Create New World" works again). **Fresh world required** if you are upgrading
  from **0.6.x** (ceiling +896 → **+1024**; or regenerate the top and clear the Distant
  Horizons cache) or from any version before **0.6** (floor is **−608**).
...
- **Adventure structures** — **When Dungeons Arise** `2.1.68` with a patched Big Globe compat
  ...
  terrain base). Structures are split into four `stattinkerer` sets (sky / sea / common /
  large_dungeon); 24 of 38 generate. Pillager outposts and large dungeons keep a 6-chunk
  buffer from CTOV villages. **0.7.1** dropped an orphaned `small_prairie_house` structure
  def that crashed "Create New World" (MC loads every structure def even when it is in no
  set; the pool is gone in WDA 2.1.68).
...
4. **Start a new world** if you are upgrading from 0.6.x (ceiling raised to **+1024**; or
   ...
   **0.7.1** itself does not change height — relaunch so the WDA compat hotfix installs.
